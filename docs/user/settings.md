...
The Android **Live shortcut** widget is the exception: opening Live Mode from that widget always auto-starts recording once the model is ready, even if this setting is off. That keeps the shortcut widget behaving like a dedicated quick-start launcher instead of a passive app opener.
