...
La limite N est **par espèce, et non globale** — si vous enregistrez 10 merles et 10 pinsons, vous conservez 20 clips. Il n’y a pas de limite globale au nombre de clips qu’un relevé peut produire.
...
- **Première session** — une alerte la première fois que chaque espèce est entendue au cours de ce relevé.
...
L'écran prêt résume la configuration active du relevé avant de commencer avec :material-play:.

## Tableau de bord du relevé en direct

L'écran Relevé en direct comporte trois onglets principaux ainsi qu'une liste de détections récentes.

### Barre supérieure

- :material-stop: — terminer le relevé
- :material-timer: — temps écoulé
- :material-help-circle-outline : — ouvre la feuille d'aide au relevé
- :material-tune : — ouvre les paramètres du relevé
...
Sous le contenu de l'onglet, le tableau de bord du relevé affiche une barre de statistiques et une liste de détections récentes. Appuyer sur une détection ouvre la superposition des détails de l’espèce.

## Opération en arrière-plan

Le mode Relevé maintient une notification persistante au premier plan visible pendant l'enregistrement afin qu'Android ne suspende pas le pipeline audio. La notification se développe pour afficher :
...
Les alertes d'espèces, lorsqu'elles sont activées, apparaissent sur un canal de notification Android distinct afin que vous puissiez les désactiver indépendamment de la notification d'enregistrement silencieux en cours. L'icône d'alerte correspond à l'icône de notification au premier plan, un oiseau monochrome, et le texte de l'alerte affiche uniquement la *raison* — *"Première détection de ce relevé"*, *"Sur votre liste de surveillance"*, *"Détecté à cet endroit avec moins de 4 % de probabilité"* — laissant le nom de l'espèce dans le titre de notification en gras là où Android l'affiche en plus grand.

## Après l'arrêt

BirdNET Live enregistre le relevé terminé et ouvre [Session Review](session-review.md).
